/*
  ...
  Thanks to Coleman Sellers and Harold Timmis for help getting it to work with the Phillips Driver
  7-31-2011
*/
...
void LCDShield::init(LcdDriver_t driver, bool colorSwap)
{
    ...
    if (EPSON == LcdDriver)
    {
        ...
        LCDData(0x03);      // Inverse page address, reverse rotation column address, column
                            // scan-direction	!!! try 0x01
...
// 2/18/2013 This Methos added by Tony Contrada in order to create arc segments in varied line
// thickness, or Filled
void LCDShield::setArc(
    int x0, int y0, int radius, int arcSegments[], int numSegments, int lineThickness, int color)
{
    // Line Thickness (Num Pixels)
    if (lineThickness == FILL)
    {
...
void LCDShield::printBMP(char image_main[2048])
{
    ...
    for (image_ix = 0; image_ix < 2048; image_ix++)
    { // for all the bits in the image
        image = image_main[image_ix];

        for (z = 0; z < 8; z++)
        { // for each bit in the byte
            if ((image & 0x80) == 0x80)
            { // if the bit is equal to 0, sets to white
                setPixel(WHITE, y, x);
            }
            else
            {
            ...
            x++; // adds one to x
            if (x == 132)
            {          // if the end of the screen is reached
